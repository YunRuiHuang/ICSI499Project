...
export class ItemAddComponent implements OnInit, AfterViewInit {
  ...
  onSubmit(){
    ...
    this.body.itemKeywords = this.body.itemName.split(' ').concat(this.body.itemLocation.split(/[,\s]+/)).map(keyword => keyword.trim()).filter(keyword => keyword !== '');
  ...
  ngAfterViewInit() {
    ...
    autocomplete.addListener('place_changed', () => {
      ...
      if (place.formatted_address) {
        this.body.itemLocation = place.formatted_address;
        console.log(this.body.itemLocation);
      }
    });
  }
